...
finance_url = requests.get('https://google.com/finance')
tree = html.fromstring(finance_url.content)
dj = tree.xpath('//div[@class = "pKBk1e"]')
print(dj[0].text)

perpoints = tree.xpath('//div[@class = "JwB6zf V7hZne"]')
print(perpoints[0].text)

baspoints = tree.xpath('//span[@class = "P2Luy Ez2Ioe"]')
print(baspoints[0].text)

#z = 0
together = tree.xpath('//*[@class = "pKBk1e" or @class = "JwB6zf V7hZne" or @class = "P2Luy Ez2Ioe"]')
# while z < len(together):
#     print(together[z].text)
#     z += 1
together = together[0:3]
together = list(map(lambda x: x.text, together))
print(together)
print(" ".join(together))
...
#     while len(re.findall(r"(<p><font size=\"\+2\">|<br>|</font></p>)", s)) > 0:
#         s = re.sub(r"(<p><font size=\"\+2\">|<br>|</font></p>)", '', s)
#     return s"""
# def normalizePrinciple(listofstrings):
#     stringstoreturn = []
#     for s in listofstrings:
#         while len(re.findall(r"(<p>|<font size=\"\+2\">|<br>|</font>|</p>)", s)) > 0:
#             s = re.sub(r"(<p>|<font size=\"\+2\">|<br>|</font>|</p>)", '', s)
#         stringstoreturn.append(s)
#     return stringstoreturn
        
# #counter 
# i = 0
# #list of principles
# principles = re.findall('<p>([\S|.|\s]{1,})</p>', text_page2)
# principles = normalizePrinciple(principles)
